...
export const esRutaPublica = (ruta) =>
  RUTAS_PUBLICAS.includes(ruta) || PATRONES_PUBLICOS.some((p) => p.test(ruta));
...
export const sinSesionEnRutaPublica = (ruta, hayUsuarioGuardado) => {
  ...
  if (PANTALLAS_DE_ACCESO.includes(ruta) || ruta.startsWith('/reset-password')) return true;
 ...
 */
export const esPuertaDeEntrada = (ruta) => ruta === '/' || ruta === '/start';
