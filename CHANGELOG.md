...
- **Las instrucciones de instalación en iOS dicen ahora dónde pulsar, y dejan de dar por hecho que estás en Safari** (issue #332): iOS no ofrece ninguna forma de instalar por código —Safari no implementa `beforeinstallprompt` ni equivalente, y sigue sin hacerlo en iOS 26—, así que ese texto es lo único que lleva a alguien a instalar la aplicación. Mostraba el icono de compartir dibujado en línea, que está bien, pero no decía dónde encontrarlo: quien no reconozca ya ese icono tiene que buscarlo, y encima no está siempre en el mismo sitio, porque en Safari vive en la barra inferior del iPhone y en la superior del iPad. Además el mismo texto se le enseñaba a cualquier navegador de iOS, y Chrome, Firefox y Edge llegan a "Añadir a pantalla de inicio" por su propio menú; no era un caso hipotético, porque el agente de usuario con el que ya se probaba el hook era precisamente Chrome en iOS. Ahora se indica la barra concreta en Safari, distinguiendo iPhone de iPad, y al resto se les remite al menú de su navegador sin prometerles una posición que no se puede dar por cierta. Reconocer esos navegadores exige buscar su propia marca en el agente de usuario, porque todos son WebKit por obligación y arrastran "Safari" en él.
